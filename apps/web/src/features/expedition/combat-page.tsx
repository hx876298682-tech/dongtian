...
export function CombatPage(): ReactElement {
  ...
  return <section className="behavior-layout behavior-layout--expedition" aria-label="历练总览">
    <header className="behavior-panel behavior-panel--hero" aria-label="历练标题"><div><p className="page-card__eyebrow">历练 · 自动战斗</p><h3 className="page-card__title">选择怪物，开始挂机</h3><p className="page-card__copy">先选地图，再点击怪物即可开始战斗挂机；战斗会和其他行动一样持续结算，收益显示在顶部行为栏。</p></div><span className="behavior-panel__realm">{regions.length} 张地图 · {11} 只怪物</span></header>
    <div className="behavior-panel behavior-panel--regions" aria-label="历练地图"><div className="behavior-panel__heading"><h4>地图</h4><span>{regions.length} 个区域</span></div><div className="behavior-region-list">{regions.map((region) => <button key={region.id} className={`behavior-region ${region.id === selectedRegion?.id ? 'behavior-region--selected' : ''}`} type="button" aria-pressed={region.id === selectedRegion?.id} onClick={() => selectRegion(region.id)}><span className="behavior-region__header"><strong>{region.label}</strong><small>{region.stageLabel}</small></span><span className="behavior-region__description">{region.description}</span></button>)}</div></div>
    <div className="behavior-panel behavior-panel--resources" aria-label="历练怪物"><div className="behavior-panel__heading"><div><h4>{selectedRegion?.label ?? '历练地图'}</h4><p>点击怪物开始战斗</p></div><span>{monsters.length} 只怪物</span></div><div className="behavior-resource-list">{monsters.map((monster) => <CombatMonsterCard key={monster.id} monster={monster} action={actionForMonster(monster, actions)} selected={monster.id === selectedMonster?.id} starting={startingActionId === monster.actionId} onActivate={() => { setSelectedMonsterId(monster.id); if (actionForMonster(monster, actions) !== null) void startCombat(monster); }} />)}</div></div>
