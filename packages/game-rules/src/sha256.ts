...
function gamma0(value: number): number {
  return rotateRight(value, 7) ^ rotateRight(value, 18) ^ (value >>> 3);
}

function gamma1(value: number): number {
...
export function sha256Bytes(input: Uint8Array): Uint8Array {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      schedule[index] = (
        gamma1(schedule[index - 2] ?? 0)
        + (schedule[index - 7] ?? 0)
        + gamma0(schedule[index - 15] ?? 0)
        + (schedule[index - 16] ?? 0)
      ) >>> 0;
    }
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary2 = (sigma0(a) + majority(a, b, c)) >>> 0;
      h = g;
      g = f;
      f = e;
      e = (d + temporary1) >>> 0;
