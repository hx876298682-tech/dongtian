...
const canonicalize = () => ({
  ...
  recipes: recipes.map((recipe) => { const result = canonicalRecord(recipe, ['id', 'building_id', 'interval_parameter', 'output_resource', 'output_parameter', 'input_parameters']); result.input_parameters = canonicalRecord(recipe.input_parameters, recipe.id === 'alchemy_basic' ? ['spirit_herb', 'spirit_stone'] : ['spirit_ore', 'spirit_wood', 'spirit_stone']); return result; }),
});
...
const manifest = { config_version: '1.0.0-frozen', parameter_sha256: '7113fe72dd40ed36869408551f5f20a9e72e83af519b4a03d657296ed4987f75', schema_version: '1.1', status: 'frozen_v1', content_files: ['maps.json', 'equipment.json', 'recipes.json'], content_sha256: hash, asset_manifest: assetManifest, name_pool_ids: namePoolIds, appearance_tag_ids: appearanceTagIds, stat_template_ids: statTemplateIds };
